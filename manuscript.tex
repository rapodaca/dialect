...
Dialect molecules inherit behavior from an underlying \textit{molecular tree}. A tree is a data structure comprised of nodes. Each node is associated with zero or more child nodes through a directed edge. A molecular tree is a specialized tree onto which chemically-relevant metadata have been overlaid. The presence of this overlay allows nodes and edges to represent atoms and bonds, respectively.

A molecular tree is related to the more widely-used concept of a \textit{molecular graph}~\cite{balaban:1985}. There are two main difference: (1) bonds are undirected in molecular graphs but directed in molecular trees; and (2) cycles are possible in molecular graphs but not in molecular trees. The difference might appear to render trees unsuitable to represent molecules, in which cycles often play a prominent structural role. But as the next sections demonstrate, the differences between molecular trees and graphs are not so large as they might at first appear.

A Dialect molecule is a molecular tree having zero or more nodes and zero or more edges. The empty molecule, devoid of atoms and bonds, is therefore allowed. Dialect imposes no upper bound on the number of nodes or edges, although limitations in hardware and software may impose a practical upper bound on molecular size.

\section*{Data Model}

Dialect uses several abstractions collectively known as the \textit{data model}. The data model defines metadata attributes associated with the components of a molecular tree. These attributes are themselves composed of \textit{primitives} (Table~\ref{table:primitives}), or irreducible data types.

% TODO: completely re-work this to fit with the exact model being used by the reference implementation
...
\begin{tabular}{l l l}
    ...
    \hline
    \ttt{Option<T>} & Optional Value & \ttt{None}, \ttt{T} \\
    \ttt{Three} & Integer count & 1..4 \\
    \ttt{Dec} & Integer count &0..10 \\
    \ttt{SignedDec} & Integer count & -9..10 \\
    \ttt{Cent} & Integer count & 1..100 \\
    \ttt{Thou} & Integer count & 1..1000 \\
    ...
    \ttt{Implicit} & Implicit hydrogen count flag & - \\
...
\begin{tabular}{l L L l}
    ...
    \ttt{hydrogens} & Hydrogen count & \ttt{Implicit}, \ttt{Dec} & 0 \\
    \ttt{configuration} & Configuration & \ttt{Option} \ttt{<Configuration>} & \ttt{None} \\
    \ttt{charge} & Formal charge & \ttt{SignedDec} & 0 \\
    \ttt{isotope} & Mass number & \ttt{Option<Thou>} & \ttt{None} \\
...
A molecular tree is described at the lowest level by its constitution. In Dialect, constitution comprises those attributes that associate atoms in bonding relationships, and which locate nucleons and electrons at specific atoms and bonds. All other attributes lie outside the scope of constitution. Atomic attributes related to constitution are: \ttt{element}; \ttt{hydrogens}; and \ttt{charge}. Bond attributes related to constitution are \ttt{order} and \ttt{elided}.

The attributes \ttt{element} and \ttt{isotope} characterize an atom's nucleus. The \ttt{element} attribute is an optional one- or two-letter character sequence selected from the set designated by the IUPAC~\cite{periodicTable}. When IUPAC authorizes additional symbols, they will become valid values for the \ttt{element} attribute. If the elemental identity of an atom is unknown, its \ttt{element} attribute must equal \ttt{None}. The \ttt{isotope} attribute is an optional integer value representing an atom's nuclear mass number, where mass number is the sum of proton and neutron count. Setting the \ttt{isotope} property to \ttt{None} means that the element's isotopic composition equals natural abundance. Otherwise, the lower bound on the value of \ttt{isotope} is one. This lower limit allows for such physically nonsensical states such as atoms of negative implied neutron count (e.g., carbon-5). Implementations may reject such atomic states as invalid.
...
Certain aspects of Dialect rely on \textit{node order}. An order is a property of a set that allows its members to be sorted through pairwise comparisons. Node order is the order in which the nodes of a molecular tree are added during depth-first construction. A node added before another node is said to \enquote{precede} it. A node added after another node is said to \enquote{succeed} it. Using node order, the nodes of a molecular tree can be sorted from first to last.

\begin{figure}
    \centering
    \includegraphics{filler}
    \caption{Node Order. Nodes in a molecular tree have the order in which they were added through depth-first construction.}
    \label{fig:node-order}
\end{figure}

Some generalizations about node order follow from these definitions. The root atom, uniquely lacking a parent, precedes every other node. Every parent node precedes its children. The first child added to a given parent precedes all of its siblings. Moreover, this relationship is transitive. If a node precedes (or succeeds) a sibling, then all of its descendants will as well.

\section*{Decoupling Bonding from Connectivity}

A bond performs two important tasks within a molecular tree: it creates a bond between two atoms and it creates a connection between two nodes. Most of the time, performing both tasks together is exactly what's needed. In two cases, however, doing so limits the range of molecules that can be encoded.

The first case for decoupling involves cycles. A molecular tree is by definition devoid of cycles, yet cycles are ubiquitous, crucial elements of molecular structure. To encode cycles, a molecular tree needs a way unambiguously record the presence of cycle-closure bonds without creating the corresponding connection. Dialect accomplishes this goal through \textit{cut bonding}.

\begin{figure}
    \centering
    \includegraphics{filler}
    \caption{Cut Bonding. A bonding relationship represented by two nodes and two edges.}
    \label{fig:cut-bonding}
\end{figure}

Cut bonding represents a bond as two semi-independent node/bond pairs. These pairs are distinguished from each other with the designations \enquote{left} and \enquote{right.} The left cut node and its parent precede the right cut node and its parent. Left and right side cut nodes are attached to their respective atom parents through equivalent bonds, but not to each other. In this way a bonding arrangement is established without creating a connection. Unlike an atom, a cut node has zero children and must have an atomic parent. As such a cut node can not serve as the root of a molecular tree.

Each cut node is associated with a single attribute, \ttt{index}. The value of this attribute ranges from one to 99, inclusive (\ttt{Cent} type). Left and right cut nodes identify themselves as a pair by using identical \ttt{index} attributes. A single molecular tree can support far more than 99 cuts through index reuse. Any left cut node succeeding the last right cut node having a particular index can reuse it. The next succeeding cut node with the index must by definition be the matching right hand side.

\begin{figure}
    \centering
    \includegraphics{filler}
    \caption{Index reuse. This allows for many more than 99 cuts to be used within a single molecular tree.}
    \label{fig:index-reuse}
\end{figure}

The second case for decoupling the formation of bonds and connections is multiple molecules. Dialect supports these uses through \textit{split edges}. A split edge allows two atoms to be connected within a tree, but without creating a bonding relationship. Split edges can be used whenever multiple molecules need to be encoded within the same molecular tree. One application would be molecules with ionic bonding. The atoms are not bound covalently, and so fall outside the scope of the VB model. Another application would be non-ionic complexes. No bonding relationship whatsoever is implied between atoms connected through a split edge.

\begin{figure}
    \centering
    \includegraphics{filler}
    \caption{Split Edge. Connection without a bonding relationship.}
    \label{fig:split-edge}
\end{figure}

Split edges and cut bonding accomplish the same goal, namely the decoupling of connectivity and bonding, but for opposite purposes. Whereas split edges connect two atoms without bonding them, cut bonds create a bonding relationship between two nodes without connecting them. This decoupling complicates the analysis of molecular trees. For example, the atom succeeding a split edge behaves in some situations as if it were a root node. When tallying the bonds surrounding such a node, only children are considered. Likewise, cut nodes behave as if they were atoms in the sense that they can be associated with a bond. Within these and several other situations, analysis of bonding and connectivity relationships within molecular trees need to explicitly account for the possibility of connection-free bonds and bond-free connections. 
...
To support this system, each bond carries an optional \ttt{direction} attribute (\enquote{direction}). When defined, direction may assume one of two values: \ttt{Up} or \ttt{Down}. These values refer to a geometrical model in which a parent node and its child are placed along a vertical axis. If the bond between parent and child has the direction \ttt{Up}, then the child is moved in the upward direction. If the bond between parent and child is \ttt{Down}, the child is moved downward.

Using this protocol, the conformation for a double bond flanked by two or more PPBs can be determined. The process begins by identifying the parent node. This node has the lowest order of the conformational assembly, meaning that it was added first. The parent node is placed on a vertical axis. The children of the parent node are then examined in order. The child connected through the first directional bond is then positioned as described above, either above or below the parent as determined by the value of the bond's direction attribute.

Next, grandchildren are examined in order. Should a PPB be found, its direction must match that of the bond between parent and child. Otherwise an error is generated. A double bond must be found, or an error is reported. On finding the double bond, the corresponding grandchild is placed to the right of the child.

The process finishes by iterating the great-grandchildren of the double-bond grandchild, in order. The first PPB is placed as before, above the grandchild if the direction is \ttt{Up}, or below if the direction is \ttt{Down}. If an additional PPB is present, it must have a direction opposite to the one previously found. Otherwise, an error is reported.

The result will be equivalent to one of two diagrams, which for convenience and to avoid confusion are given the labels \textit{syn} (both vertical lines above or below the double bond) and \textit{anti} (vertical lines on opposite sides of the double bond).
...
Consider the problem of encoding of PPBs for (\textit{E})-2-butene. The parent atom, a primary carbon, is first identified. A direction of \ttt{Up} for the PPB joining it to its child is arbitrarily chosen. A double bond connects child to grandchild. Just one bond connects grandchild to great-grandchild. Because the \textit{anti} arrangement is desired, the value for direction must be \ttt{Up}. In this example, the directions for both PPBs could also have been equivalently set to \ttt{Down}.
...
\begin{itemize}
    \item{Overspecification. This error occurs when children are forced into the same quadrant of a diagram.}
...
A reader that detects any of these error conditions must abort the molecular tree being read and report the error state. It is possible for a conformation to be both overspecified and underspecified.
...
An exception to the underspecification rule applies in the case of conjugated polyenes. Here, a PPB may be present into the terminal double bond but not out of it. No error is reported in such a case and the conformation of the terminal double bond remains undefined.
...
A configuration is fully specified by its \textit{configurational descriptor}. A configurational descriptor is a template for the relative three-dimensional positioning of neighbors about a central atom. Dialect supports two configurational descriptors: \ttt{Left} and \ttt{Right}. These descriptors constrain the neighbors of a central atom to the vertices of a tetrahedron.

A descriptor generates a tetrahedral arrangement as follows. First, the parent (predecessor) bound to an atom of interest is identified. Next, the frame of reference is shifted by sighting down the bond from parent to child. An equilateral triangle is then inscribed bout the central atom. Grandchildren are then iterated in order. If the configurational descriptor is \ttt{Left}, children are arranged counterclockwise. Otherwise, children are arranged clockwise.
...
A modified procedure is used for atoms bearing one virtual hydrogen and three atomic substituents. The implied bond to the virtual hydrogen is considered to precede the others. Sighting down this bond, the parent node is placed at an arbitrary position. Children are then iterated counterclockwise (\ttt{Left}) or clockwise (\ttt{Right}).
...
A root atom bearing a configurational descriptor is handled with one more modification. The frame of reference is shifted to sight along the direction of the first child to the parent. The remaining children are then iterated in order either counterclockwise (\ttt{Left}) or clockwise (\ttt{Right}).
...
A configurational descriptor may be applied without regard to bond type. In other words, single, double, triple, and partial parity bond types can all be present. The only requirement that must be satisfied by a central atom is a connected neighbor count of four, or three when a virtual hydrogen is present.
...
The \ttt{<cut>} non-terminal (\enquote{cut}) can takes two forms. A digit non-terminal can be used, enabling single-digit cut indexes in the range one through nine, inclusive. Double- or single-digit cut indexes, supporting the values ten through 99 inclusive, are available by prepending the percent character (\ttt{\%}). For the latter variant, leading zeros are disallowed, meaning that the sequence \enquote{\%07} must generate an error.

\begin{lstlisting}
<cut> ::= <nonzero> | "%" <nonzero> <digit>
\end{lstlisting}
